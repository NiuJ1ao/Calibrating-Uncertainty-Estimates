...
def maximum_calibration_error(predictions, confidences, references, device="cpu"):
    _, _, bin_accs, bin_avg_confs = partition_bins(predictions, confidences, references, device)
    
    mce = torch.max(torch.abs(bin_accs - bin_avg_confs))
    return mce
